...
绘图、编辑与遍历类工具一律作用于 **当前空间**，跟随 `TILEMODE` / 当前布局，
与你在命令行敲 `LINE` 的落点一致：

| 当前布局 | 实体建在 | `query_entities` / `select` 扫的 |
|---|---|---|
| `Model` | 模型空间 | 模型空间 |
| 某个布局 | **该布局的图纸空间** | 该布局的图纸空间 |

所以 `set_layout` 不只是切个标签页，它同时改变了「画在哪、查哪」——回执里会写明这一点。
`draw_*` 的返回是 `handle=245（模型空间）` 这样的形式，`query_entities` 的结果里有 `space` 字段：
**东西落在哪个空间，永远跟着结果一起报**。

切到布局后查不到模型空间的实体不是缺陷，与 AutoCAD 的 `SELECT` 行为一致；要看模型空间就先
`set_layout Model`。

> 这一段是补出来的。早先所有工具都写死了模型空间，于是 Agent 切到布局画图框，图元进了模型空间，
> 而回执只有一个 `handle=28A`——它以为画成了，九个回合之后才靠读 DXF 组码 67 发现不对。
> 实现见 `src/AcadMcp.Plugin/Acad/Space.cs`。

---

## 已知限制

- 在布局里双击进**浮动视口**（MSPACE，`CVPORT≠1`）时，AutoCAD 自身的命令会画进模型空间，
  而本插件仍按图纸空间处理（判据是 `Database.CurrentSpaceId`，它只看 TILEMODE）。
  本插件没有进出 MSPACE 的工具，正常路径走不到那个状态
